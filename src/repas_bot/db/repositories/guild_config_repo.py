...
class GuildConfigRepo:
    ...
    async def get(self, guild_id: int):
        cur = await self.db.execute("SELECT guild_id, admin_role_name, timezone, default_meal_price_cents, menu_channel_id, poll_channel_id FROM guild_config WHERE guild_id=?", (guild_id,))
    ...
    async def set_poll_channel(self, guild_id: int, channel_id: int) -> None:
        await self.db.execute("""
        INSERT INTO guild_config(guild_id, poll_channel_id)
        VALUES(?, ?)
